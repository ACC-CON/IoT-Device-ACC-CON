...
library Sha512 {
	...
	function bytesToBytes8(bytes memory b, uint offset) public pure returns (bytes8) {
		bytes8 out;
		for (uint i = 0; i < 8; i++) {
    ...
    function cutBlock(bytes memory data, uint blockIndex)
        public
        pure
    ...
    function digest(bytes memory data) public pure returns (uint64[8] memory) {
		// 中间哈希
        uint64[8] memory H = [
			0x6a09e667f3bcc908,
			0xbb67ae8584caa73b,
